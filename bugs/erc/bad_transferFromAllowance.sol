...
abstract contract ERC20 {
    ...
    function transferFrom(
        ...
        // @bug: invert from <> msg.sender on allowance
        uint256 allowed = allowance[msg.sender][from]; // Saves gas for limited approvals.

        if (allowed != type(uint256).max) allowance[msg.sender][from] = allowed - amount;
